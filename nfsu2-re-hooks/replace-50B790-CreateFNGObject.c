...
struct FNGObject* CreateFNGObject(int fnghash, struct FNGInfo *fngInfo, int arg2)
{
	...
	logging(buf, sprintf(buf, "CreateFNGObject showing for hash: %p", fnghash));
	...
	for (i = 0; i < 180; i++) {
		if (cihash(fngdata[i].name) == fnghash) {
			fng = fngdata + i;
			logging(buf, sprintf(buf, "CreateFNGObject matching fng name: %s", fng->name));
			if (fng->initializeHandler) {
				...
				data.arg2 = arg2;
				logging(buf, sprintf(buf, "CreateFNGObject calling initializeHandler"));
				result = fng->initializeHandler(&data);
				logvalue("CreateFNGObject", "result", result);
			} else {
				result = 0;
				logging(buf, sprintf(buf, "CreateFNGObject initializeHandler is NULL"));
			}
			return result;
		}
	}

	logging(buf, sprintf(buf, "CreateFNGObject no matching fng found?"));
	return 0;
}
